...
- The shared bottom composer (Codex, Terminal, and Workflow tabs) has a translate button next to the attachment button: it translates the draft from the user language (Chinese) to the LLM language (English) via `POST /api/translate`, swaps the translated text into the input for editing/sending, and toggles back to the original draft on a second click; editing the draft or sending it clears the translation state.
