...
const requestedServerLogLevel = (
  process.env.CODEXDECK_SERVER_LOG_LEVEL || ""
).toLowerCase();
const serverLogLevel = validLogLevels.has(requestedServerLogLevel)
  ? requestedServerLogLevel
  : "warn";

if (requestedServerLogLevel && !validLogLevels.has(requestedServerLogLevel)) {
  console.warn(
    `[PINO] Invalid CODEXDECK_SERVER_LOG_LEVEL="${process.env.CODEXDECK_SERVER_LOG_LEVEL}", falling back to "warn"`,
  );
}
    ...
    log: (object: any) => {
      ...
      return {
        ...
        localTime: formatLocalTime(
          typeof object.time === "number" ? object.time : undefined,
        ),
      };
    },
  },
...
export const fileConsolidatedLogger =
  process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING &&
  consolidatedLogFile
    ? pino({
        level: serverLogLevel,
        ...
        timestamp: () =>
          `,"time":${Date.now()},"localTime":"${formatLocalTime()}"`,
      })
    : undefined;
