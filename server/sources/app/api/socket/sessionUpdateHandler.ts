...
export function sessionUpdateHandler(
    ...
    async (data: any, callback: (response: any) => void) => {
      try {
        ...
        log(
          ...
          `Error in update-metadata: ${error}`,
        );
        if (callback) {
          callback({ result: "error" });
        }
  ...
  socket.on(
    "session-alive",
    async (data: { sid: string; time: number; thinking?: boolean }) => {
      try {
        // Track metrics
