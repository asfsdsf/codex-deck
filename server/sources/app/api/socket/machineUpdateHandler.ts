...
export function machineUpdateHandler(
  ...
  socket.on(
    "machine-alive",
    async (data: { machineId: string; time: number }) => {
      try {
        // Track metrics
        ...
        if (data.machineId !== boundMachineId) {
          return;
        }
        ...
        const isValid = await activityCache.isMachineValid(
          data.machineId,
          userId,
        );
        if (!isValid) {
  ...
  socket.on(
    ...
    async (data: any, callback: (response: any) => void) => {
      try {
        ...
        log(
          { module: "websocket", level: "error" },
          `Error in machine-update-metadata: ${error}`,
        );
        if (callback) {
  ...
  socket.on(
    ...
    async (data: any, callback: (response: any) => void) => {
      try {
        ...
        log(
          { module: "websocket", level: "error" },
          `Error in machine-update-state: ${error}`,
        );
        if (callback) {
          callback({ result: "error", message: "Internal error" });
        }
