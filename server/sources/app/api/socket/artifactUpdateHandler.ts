      ...
      try {
        ...
        if (
          ...
          (typeof header.data !== "string" ||
            typeof header.expectedVersion !== "number")
        ) {
          if (callback) {
        ...
        const headerMismatch =
          header && currentArtifact.headerVersion !== header.expectedVersion;
        const bodyMismatch =
          body && currentArtifact.bodyVersion !== body.expectedVersion;
        ...
        log(
          { module: "websocket", level: "error" },
          `Error in artifact-update: ${error}`,
        );
        if (callback) {
          callback({ result: "error", message: "Internal error" });
        }
  ...
  socket.on(
    ...
    async (
      ...
      try {
        ...
        log(
          { module: "websocket", level: "error" },
          `Error in artifact-create: ${error}`,
        );
        if (callback) {
          callback({ result: "error", message: "Internal error" });
        }
  ...
  socket.on(
    ...
    async (
      ...
      try {
        ...
        const updSeq = await allocateUserSeq(userId);
        const deletePayload = buildDeleteArtifactUpdate(
          artifactId,
          updSeq,
          randomKeyNaked(12),
        );
        ...
        log(
          { module: "websocket", level: "error" },
          `Error in artifact-delete: ${error}`,
        );
        if (callback) {
          callback({ result: "error", message: "Internal error" });
        }
