    ...
    async (request, reply) => {
      ...
      if (!tokenData) {
        ...
        return reply.redirect(
          "https://app.codexdeck.invalid?error=invalid_state",
        );
      }
      ...
      if (!clientId || !clientSecret) {
        return reply.redirect(
          "https://app.codexdeck.invalid?error=server_config",
        );
      }
      ...
      try {
        ...
        if (tokenResponseData.error) {
          return reply.redirect(
            `https://app.codexdeck.invalid?error=${encodeURIComponent(tokenResponseData.error)}`,
          );
        }
        ...
        if (!userResponse.ok) {
          return reply.redirect(
            "https://app.codexdeck.invalid?error=github_user_fetch_failed",
          );
        }
        ...
        return reply.redirect(
          `https://app.codexdeck.invalid?github=connected&user=${encodeURIComponent(userData.login)}`,
        );
      } catch (error) {
        log(
          { module: "github-oauth" },
        ...
        return reply.redirect(
          "https://app.codexdeck.invalid?error=server_error",
        );
      }
    },
  );
