...
export function artifactsRoutes(app: Fastify) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        log(
          { module: "api", level: "error" },
          `Failed to get artifacts: ${error}`,
        );
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (existingArtifact) {
          ...
          log(
            { module: "api", artifactId: id, userId },
          ...
          return reply.send({
            ...
            dataEncryptionKey: privacyKit.encodeBase64(
              existingArtifact.dataEncryptionKey,
            ),
  ...
  app.post(
    ...
    async (request, reply) => {
      const userId = request.userId;
      const { id } = request.params;
      const { header, expectedHeaderVersion, body, expectedBodyVersion } =
        request.body;
