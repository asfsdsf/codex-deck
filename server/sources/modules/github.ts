...
function registerWebhookHandlers() {
  ...
  webhooks.on(
    "push",
    async ({ id, name, payload }: EmitterWebhookEvent<"push">) => {
      log(
  ...
  webhooks.on(
    "issues",
    async ({ id, name, payload }: EmitterWebhookEvent<"issues">) => {
      log(
  ...
  webhooks.on(
    ...
    async ({
      ...
      name,
      payload,
    }: EmitterWebhookEvent<"star.created" | "star.deleted">) => {
  ...
  webhooks.on(
    "repository",
    async ({ id, name, payload }: EmitterWebhookEvent<"repository">) => {
      log(
  ...
  webhooks.onAny(async ({ id, name, payload }: EmitterWebhookEvent) => {
    log(
      { module: "github-webhook", event: name as string },
      `Received webhook event: ${name}`,
      { id },
    );
  });
