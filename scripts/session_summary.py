#!/usr/bin/env python3
"""Summarize a Codex session JSONL file similar to codex-deck viewport behavior."""
    ...
    parser.add_argument(
        ...
        help=(
         "Path to CODEX_HOME. If omitted, uses CODEX_HOME env var. If that is unset, "
            "uses codex-deck's default (~/.codex on Unix/macOS and %%USERPROFILE%%\\.codex on Windows)."
        ),
    )
    parser.add_argument(
        "--line-limit",
...
def resolve_codex_home(args_codex_home: str | None) -> Path:
    ...
    # codex-deck uses join(homedir(), ".codex") regardless of OS.
